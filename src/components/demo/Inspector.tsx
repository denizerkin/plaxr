...
import { Bot, Plus, Settings, Save } from 'lucide-react';
    ...
    <div className="w-80 bg-slate-900 border-l border-slate-800 flex flex-col h-full z-20">
      ...
      <div className="flex-1 overflow-y-auto">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <select className="col-span-2 w-full bg-slate-800 border border-slate-700 rounded px-2 py-1.5 text-xs text-white focus:outline-none focus:border-cyan-500">
                  <option>BridgePort Serie 1</option>
                  <option>BridgePort Serie 2</option>
                  <option>BridgePort Serie 3</option>
                </select>
              </div>
            </div>

            {/* Scenario template - UPDATED HERE */}
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1">
                Scenario template
              </label>
              <select className="w-full bg-slate-800 border border-slate-700 rounded px-2 py-1.5 text-xs text-white focus:outline-none focus:border-cyan-500">
                <option>Head Tramming Procedure</option>
                <option>Spindle Belt Replacement</option>
                <option>Gib Adjustment & Lubrication</option>
                <option>Quill Feed Diagnostics</option>
              </select>
            </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <div className="space-y-1">
                  ...
                  <select className="w-full bg-slate-800 border border-slate-700 rounded px-2 py-1 text-[11px] text-white focus:outline-none focus:border-cyan-500">
                    ...
                    <option>Industrial Floor</option>
        ...
        <div className="p-4">
            ...
            <div className="space-y-2 text-xs">
                {[
                'Dial Indicator (Tramming)',      
                'Hotspots: Spindle, Quill, Knee', 
                'Drawbar & Collet Interaction',   
                'Belt Tension Feedback',          
                'Quiz: Calculate RPM / Feeds',    
                ].map((comp) => (
                <div
                    key={comp}
      ...
      <div className="p-4 bg-slate-800/50 border-t border-slate-800">
        ...
        <p className="text-xs text-slate-300">
            You&apos;re designing a maintenance module for the{" "}
            <span className="text-cyan-400">
            Bridgeport Series 1
            </span>
            . Want me to generate a flow for checking spindle runout, lubricating the ways, and adjusting belt tension?
        </p>
        </div>
