import { MoreHorizontal, Mail, Search, Filter, Plus, ChevronLeft, ChevronRight } from 'lucide-react';

const StudentsView = () => {
    const students = [
        { id: 1, name: 'Alice Johnson', email: 'alice@example.com', progress: 85, lastActive: '2 hours ago', status: 'Active', role: 'Student' },
        { id: 2, name: 'Bob Smith', email: 'bob@example.com', progress: 42, lastActive: '1 day ago', status: 'Active', role: 'Student' },
        { id: 3, name: 'Charlie Brown', email: 'charlie@example.com', progress: 12, lastActive: '5 days ago', status: 'Inactive', role: 'Student' },
        { id: 4, name: 'Diana Prince', email: 'diana@example.com', progress: 98, lastActive: '10 mins ago', status: 'Active', role: 'Team Lead' },
        { id: 5, name: 'Evan Wright', email: 'evan@example.com', progress: 0, lastActive: 'Never', status: 'Pending', role: 'Student' },
        { id: 6, name: 'Fiona Gallagher', email: 'fiona@example.com', progress: 65, lastActive: '3 hours ago', status: 'Active', role: 'Student' },
    ];
        ...
        <div className="flex-1 bg-slate-950 p-8 overflow-y-auto">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                <div>
                    ...
                    <p className="text-slate-400 mt-1">Manage your class roster and track progress.</p>
                </div>
                <div className="flex items-center gap-3">
                    <button className="flex items-center px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg border border-slate-700 transition-colors text-sm font-medium">
            ...
            <div className="bg-slate-900/40 border border-slate-800 rounded-xl overflow-hidden backdrop-blur-sm">
                                ...
                                <tr key={student.id} className="group hover:bg-slate-800/40 transition-colors">
                                    <td className="px-6 py-6">
                                        <div className="flex items-center">
                                            ...
                                            <div>
                                                <div className="font-medium text-white group-hover:text-cyan-400 transition-colors">{student.name}</div>
                                    ...
                                    <td className="px-6 py-6 text-slate-400">{student.lastActive}</td>
                                    <td className="px-6 py-6">
                                        <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border ${student.status === 'Active' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
                                            student.status === 'Inactive' ? 'bg-slate-700/30 text-slate-400 border-slate-600' :
                                                'bg-amber-500/10 text-amber-400 border-amber-500/20'
                                            }`}>
                                            <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${student.status === 'Active' ? 'bg-emerald-400' :
                ...
                <div className="px-6 py-4 border-t border-slate-800 flex items-center justify-between">
                    <div className="text-sm text-slate-500">
                        Showing <span className="font-medium text-white">1</span> to <span className="font-medium text-white">6</span> of <span className="font-medium text-white">248</span> results
                    </div>
                    <div className="flex space-x-2">
                        <button className="p-2 border border-slate-700 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 disabled:opacity-50 disabled:cursor-not-allowed">
